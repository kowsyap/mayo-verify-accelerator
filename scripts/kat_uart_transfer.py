...
DEFAULT_PORT     = 'COM5'
DEFAULT_BAUD     = 115200
DEFAULT_KAT_FILE = '../kat/KAT_R2_S2.kat'
...
# Serial helpers
# ---------------------------------------------------------------------------
def open_serial(port: str, baud: int, timeout: float = 60.0) -> serial.Serial:
    ser = serial.Serial(
        port=port,
        baudrate=baud,
...
def wait_for_sync(ser: serial.Serial):
    """Send a trigger byte to wake the device, then wait for its sync message."""
    ...
    while rawbyte != b'\n':
        rawbyte = ser.read(1)
        readstring += rawbyte.decode("utf-8")
    print(readstring)
    ser.reset_input_buffer()
...
def run_kat(ser: serial.Serial, cases: list) -> bool:
    ...
    for tc in cases:
        ...
        device_says_valid = (raw == EXPECTED_RESPONSE)
        kat_expects_valid = (tc.fields.get('VALID', '1') == '1')

        ok = (device_says_valid == kat_expects_valid)
        print(f"  {'PASS' if ok else 'FAIL'}  "
              f"device={'SIGNATURE VALID' if device_says_valid else 'SIGNATURE INVALID'}  "
              f"expected={'SIGNATURE VALID' if kat_expects_valid else 'SIGNATURE INVALID'}  "
              )
        if ok:
            passed += 1
        else:
...
def main():
    ...
    try:
        ...
        # Signal device to exit inner loop and re-sync for the next run
